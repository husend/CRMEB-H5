<template>
  <div class="order-details">
    ...
    <div class="header bg-color-red acea-row row-middle" :class="orderInfo.orderStatus === 4 ? 'on' : ''">
      <div class="pictrue" v-if="!true">
        <img :src="orderInfo.status_pic" />
      </div>
      <div class="data" :class="orderInfo.orderStatus === 4 ? 'on' : ''">
        <div class="state">{{ orderInfo._status }}</div>
        <div>
    ...
    <template v-if="true"><!-- !refundOrder -->
      <div v-show="false" class="nav">
        <div class="navCon acea-row row-between-wrapper">
          <div :class="{ on: orderInfo.orderStatus >= 1 }">
            待付款
          </div>
          <div :class="{ on: status.type === 1 }" v-if="orderInfo.shipping_type === 2">
            待核销
          </div>
          <div :class="{ on: orderInfo.orderStatus >= 5 }" v-if="orderInfo.courseAudit.hasTrainaid === 2">
            待发货
          </div>
          <div :class="{ on: orderInfo.orderStatus >= 6 }" v-if="orderInfo.courseAudit.hasTrainaid === 2">
            待收货
          </div>
          <div :class="{ on: status.type === 3 }" v-if="false">待评价</div>
          <div :class="{ on: orderInfo.orderStatus === 7 }">已完成</div>
        </div>
        <div class="progress acea-row row-between-wrapper">
          <div class="iconfont"
            :class="[orderInfo.orderStatus >= 1 ? 'icon-webicon318' : 'icon-yuandianxiao',
              orderInfo.orderStatus >= 1 ? 'font-color-red' : '']">
          </div>
          <div class="line" :class="{ 'bg-color-red': orderInfo.orderStatus > 1 }"></div>
          <div class="iconfont" v-if="orderInfo.shipping_type === 2"
            :class="[status.type === 1 ? 'icon-webicon318' : 'icon-yuandianxiao',
              status.type >= 1 && status.type != 6 && status.type != 9 ? 'font-color-red' : '']">
          </div>
          <div class="line" v-if="orderInfo.shipping_type === 2"
            :class="{'bg-color-red': status.type > 1 && status.type != 6 && status.type != 9}">
          </div>
          <div class="iconfont" v-if="orderInfo.courseAudit.hasTrainaid === 2"
            :class="[orderInfo.orderStatus >= 5 ? 'icon-webicon318' : 'icon-yuandianxiao',
              orderInfo.orderStatus >= 5 ? 'font-color-red' : '']">
          </div>
          <div class="line" v-if="orderInfo.courseAudit.hasTrainaid === 2" :class="{'bg-color-red': orderInfo.orderStatus > 5}">
          </div>
          <div class="iconfont" v-if="orderInfo.courseAudit.hasTrainaid === 2"
            :class="[orderInfo.orderStatus >= 6 ? 'icon-webicon318' : 'icon-yuandianxiao',
              orderInfo.orderStatus >= 6 ? 'font-color-red' : '']">
          </div>
          <div class="line" v-if="orderInfo.courseAudit.hasTrainaid === 2"
            :class="{'bg-color-red': orderInfo.orderStatus > 6}">
          </div>
          <div class="iconfont"
            :class="[orderInfo.orderStatus === 7 ? 'icon-webicon318' : 'icon-yuandianxiao',
              orderInfo.orderStatus === 7 ? 'font-color-red' : '']">
          </div>
        </div>
      </div>
      <div v-show="false" class="writeOff" v-if="orderInfo.shipping_type === 2 && orderInfo.paid === 1">
      ...
      <div v-show="false" class="map acea-row row-between-wrapper" v-if="orderInfo.shipping_type === 2 && orderInfo.paid === 1">
        <div>自提地址信息</div>
        <div class="place cart-color acea-row row-center-wrapper" @click="showChang">
          <span class="iconfont icon-weizhi"></span>查看位置
        </div>
      </div>
      <div class="address" v-if="orderInfo.courseAudit.hasTrainaid === 2">
        <div class="iconfont icon-dizhi"></div>
        <div>
          <div class="name">
          {{orderInfo.shippingAddress.receiverName}}
          <span class="phone">{{orderInfo.shippingAddress.receiverPhone}}</span>
        </div>
          <div>
            {{orderInfo.shippingAddress.receiverProvince}}
            {{orderInfo.shippingAddress.receiverCity}}
            {{orderInfo.shippingAddress.receiverDistrict}}
    ...
    ></OrderGoods>
    <div class="wrapper" v-if="false && orderInfo.shippingAddress">
      <div class="iconfont icon-dizhi"></div>
      <div class="text">
          <div class="name">
            {{orderInfo.shippingAddress.receiverName}}
            {{orderInfo.shippingAddress.receiverPhone}}
          </div>
          <div class="info">
            {{orderInfo.shippingAddress.receiverProvince}}
            {{orderInfo.shippingAddress.receiverCity}}
    ...
    <div class="wrapper">
      <div class="item acea-row row-between">
        ...
        <div class="conter acea-row row-middle row-right">
          {{ orderInfo.orderNo }}
          <span class="copy copy-data" :data-clipboard-text="orderInfo.orderNo">复制</span>
        </div>
      </div>
      <div class="item acea-row row-between">
        <div>下单时间：</div>
        <div class="conter">
          {{ orderInfo.gmtCreate | dateString('YYYY-MM-DD HH:mm:ss') }}
        </div>
      </div>
      <div class="item acea-row row-between">
        <div>订单类型：</div>
      ...
      <div class="item acea-row row-between">
        <div>支付状态：</div>
        <div class="conter">{{ orderInfo.orderStatus >= 2 ? "已支付" : "未支付" }}</div>
      </div>
      <div class="item acea-row row-between">
        <div>支付方式：</div>
        <div class="conter">{{ orderInfo.payType === 1 ? "微信支付" : "其他支付" }}</div>
      </div>
    ...
    <div v-if="orderInfo.courseAudit.hasTrainaid">
      <div class="wrapper" v-if="orderInfo.courseAudit.hasTrainaid === 2">
        <div class="item acea-row row-between">
          <div>配送方式：</div>
          <div class="conter">
            发货
    ...
    <div class="wrapper">
      <div class="item acea-row row-between">
        <div>支付金额：</div>
        <div class="conter">￥{{ orderInfo.pricePayable }}</div>
      </div>
  ...
  methods: {
    ...
    getOrderStatus: function() {
      let orderInfo = this.orderInfo || {},
        _status = orderInfo.orderStatus || { _type: 0 },
        status = {};
      let type = parseInt(orderInfo.orderType),
    ...
    getDetail() {
      ...
      orderDetail({orderNo: id}).then(res => {
          ...
          that.getOrderStatus();
          if (that.orderInfo.orderType === 2) {
              that.orderTypeName = "拼团订单";
              that.orderTypeNameStatus = false;
          } else if (that.orderInfo.orderType === 3) {
              that.orderTypeName = "砍价订单";
              that.orderTypeNameStatus = false;
          } else if (that.orderInfo.seckill_id > 0) {
              that.orderTypeName = "秒杀订单";
              that.orderTypeNameStatus = false;
          }
          that.system_store = res.data.system_store || {};
          that.mapKey = res.data.mapKey;
          that.setOfflinePayStatus(that.orderInfo.offlinePayStatus);
        })
        .catch(err => {
